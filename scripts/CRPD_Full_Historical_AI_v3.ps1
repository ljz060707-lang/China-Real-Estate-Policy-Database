...
# uv should use the project's .venv rather than a previously activated .venv-1.
if ($env:VIRTUAL_ENV) {
    $ProjectVenv = (Join-Path $Root ".venv")
    try {
        $ActiveVenv = [System.IO.Path]::GetFullPath($env:VIRTUAL_ENV)
        $ExpectedVenv = [System.IO.Path]::GetFullPath($ProjectVenv)
        if ($ActiveVenv -ne $ExpectedVenv) {
...
if (-not ("CRPDKeepAwakeV3" -as [type])) {
    Add-Type @"
using System;
using System.Runtime.InteropServices;
public static class CRPDKeepAwakeV3 {
