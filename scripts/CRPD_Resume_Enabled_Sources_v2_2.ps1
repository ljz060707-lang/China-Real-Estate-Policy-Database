...
function Invoke-PolicyDbTimed {
    ...
    Write-RunLog -Message "开始：policydb $($Arguments -join ' ')" -Level "INFO"
    Write-RunLog -Message "阶段超时：$TimeoutMinutes 分钟；stdout=$StdOut" -Level "INFO"

    # 不再使用 Start-Process -PassThru 读取 ExitCode。
    # Windows PowerShell 5.1 对快速退出的 console-script shim 可能返回空 ExitCode，
    # 从而把实际成功的 policydb --help 误判为失败。
    $StartInfo = New-Object System.Diagnostics.ProcessStartInfo
    $StartInfo.FileName = $PolicyDbExe
    ...
    try {
        if (-not $Process.Start()) {
            throw "无法启动policydb进程。"
        }
    }
    catch {
        $Message = "无法启动阶段：$Stage；$($_.Exception.Message)"
    ...
    # 立即异步读取两个流，避免缓冲区满后子进程死锁。
    $StdOutTask = $Process.StandardOutput.ReadToEndAsync()
    $StdErrTask = $Process.StandardError.ReadToEndAsync()
    $StartedAt = Get-Date
    $TimedOut = $false
    $HeartbeatMilliseconds = [math]::Max(500, $HeartbeatSeconds * 1000)
    ...
    while (-not $Process.WaitForExit($HeartbeatMilliseconds)) {
        ...
        if ($Elapsed.TotalMinutes -ge $TimeoutMinutes) {
            $TimedOut = $true
            Write-RunLog -Message (
                "$Stage 超过超时限制，终止进程树 PID=$($Process.Id)"
            ) -Level "ERROR"
    ...
    Set-Content -Path $StdOut -Value $Text -Encoding UTF8
    Set-Content -Path $StdErr -Value $ErrorText -Encoding UTF8
    ...
    if ($null -eq $ExitCode) {
        $Message = "无法取得真实退出码：$Stage；请查看 $StdOut 和 $StdErr"
        Write-RunLog -Message $Message -Level "ERROR"
        if (-not $ContinueOnError) {
            throw $Message
        }
    ...
    if ($ExitCode -ne 0) {
        $Message = "阶段失败（exit=$ExitCode）：$Stage；stderr=$StdErr"
        Write-RunLog -Message $Message -Level "ERROR"
        ...
        Write-RunLog -Message "完成：$Stage" -Level "INFO"
    }
...
function Run-CityYear {
    ...
    elseif ($RetryableBefore -eq 0) {
        Write-RunLog -Message "$CityName $Year 已无pending和可重试错误，跳过初始扫描。" -Level "INFO"
    }
...
Write-RunLog -Message ("从现有已启用来源补扫阶段继续。日志：{0}" -f $LogRoot) -Level "INFO"
...
for ($CityIndex = $StartCityIndex; $CityIndex -le $EndCityIndex; $CityIndex++) {
    ...
    for ($Year = $StartYear; $Year -le $EndYear; $Year++) {
        try {
            ...
            Write-RunLog -Message "$($City.city_name) $Year 控制器异常：$Message" -Level "ERROR"
...
Write-RunLog -Message "全部指定城市—年份扫描阶段结束，开始归档与后处理。" -Level "INFO"
