...
if (-not ("CRPDCityKeepAwakeV2" -as [type])) {
    Add-Type @"
using System;
using System.Runtime.InteropServices;
public static class CRPDCityKeepAwakeV2 {
...
function Invoke-CrawlShard {
    ...
    if ([string]::IsNullOrWhiteSpace($JobId)) {
        Add-ShardState -TaskId $TaskId -ShardStart $ShardStart -ShardEnd $ShardEnd `
            -JobId "" -JobStatus "missing_job_id" -CandidateCount 0 -Fetched 0 `
            -Failed 0 -DocumentVersions 0 -CapHit $false -TerminalState "failed" `
            -Note "CLI未返回job_id；这不是零政策，查看 $LogFile"
        throw "抓取命令没有返回job_id，已停止以避免把空输出误记为完成：$LogFile"
    }
    ...
    if ($JobId) {
        ...
        if ($StatusResult.ExitCode -ne 0) {
            throw "无法读取任务状态：$JobId；查看 $StatusLog"
        }

        $State = ConvertFrom-LastJsonObject -Text $StatusResult.Text
        if (-not $State -or -not $State.status) {
            throw "任务状态输出无法解析：$JobId；查看 $StatusLog"
        }

        $JobStatus = [string]$State.status
        if ($State.counters) {
    ...
    elseif ($CandidateCount -eq 0 -and $Fetched -eq 0 -and $DocumentVersions -eq 0) {
        $TerminalState = "needs_review"
        $Note = "任务真实执行但没有候选；不能据此认定该月份零政策，需要核验来源覆盖与分页证据。"
    }
    elseif ($Failed -gt 0) {
        $TerminalState = "needs_review"
...
[void][CRPDCityKeepAwakeV2]::Start()
try {
    ...
    # Fail fast if the console entry point is not actually executing.
    $HelpProbe = Invoke-NativeLogged `
        -Arguments @("--help") `
        -LogFile (Join-Path $LogRoot "${SafeCity}_cli_help.log")
    if ($HelpProbe.ExitCode -ne 0 -or $HelpProbe.Text -notmatch "Usage|用法|policydb") {
        throw "policydb CLI入口未正常执行：$PolicyDbExe"
    }
